/**
 * `ecobuilder-plugin` CLI entry.
 *
 * Usage:
 *   ecobuilder-plugin init <name>
 *   ecobuilder-plugin build [<plugin-dir>]
 *   ecobuilder-plugin dev   [<plugin-dir>] [--uploads <path>]
 *
 * Run via Bun:
 *   bun run ecobuilder-plugin <cmd>
 *
 * The CLI lives inside the SDK so plugin authors get the same code that
 * powers the host's `bun run ecobuilder-plugin` script. No HTTP, no auth, no env
...
function printHelp(): void {
  console.log(`ecobuilder-plugin — Ecobuilder plugin CLI
...
Examples:
  ecobuilder-plugin init acme.confetti
  ecobuilder-plugin init acme.seo --kind content-editor
  ecobuilder-plugin lint acme.confetti
  ecobuilder-plugin build acme.confetti
  ecobuilder-plugin dev acme.confetti
  ecobuilder-plugin dev --uploads ../instatic/uploads
`)
}
...
async function main(): Promise<void> {
  ...
  if (command === 'init') {
    ...
    if (!name) {
      console.error('Usage: ecobuilder-plugin init <name> [--kind=module|content-editor]')
      process.exit(1)
    }
    ...
    console.log(`  cd ${created.split('/').pop()} && ecobuilder-plugin dev`)
    return
  }
