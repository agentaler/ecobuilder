/**
 * `ecobuilder-plugin lint` — unit tests for the lint pipeline.
 *
 ...
 *  • Missing or malformed `ecobuilder-plugin.config.ts` is surfaced cleanly
 ...
 * of `ecobuilder-plugin.config.ts` can resolve `@core/plugin-sdk` via the host's
...
/**
 * Helper: write a minimal `ecobuilder-plugin.config.ts` so the lint reaches the
...
describe('ecobuilder-plugin lint', () => {
  ...
  it('reports a config-level error when ecobuilder-plugin.config.ts is missing', async () => {
