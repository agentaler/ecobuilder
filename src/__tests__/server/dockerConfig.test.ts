...
describe('self-host docker config', () => {
  ...
  it('defines a production Docker image that builds assets before runtime startup', () => {
    ...
    expect(dockerfile).toContain('ARG ECOBUILDER_VERSION=dev')
    expect(dockerfile).toContain('LABEL org.opencontainers.image.version="${ECOBUILDER_VERSION}"')
  ...
  it('defines production environment variables required by the compose stack', () => {
    ...
    expect(env).toContain('ECOBUILDER_SECRET_KEY=')
    expect(env).toContain('TRUSTED_PROXY_CIDRS=')
    expect(compose).toContain('ECOBUILDER_SECRET_KEY:')
