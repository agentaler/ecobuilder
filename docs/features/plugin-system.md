...
A plugin is a zip package containing a `plugin.json` manifest and one or more bundled JavaScript entrypoints. The SDK CLI usually authors that zip from `ecobuilder-plugin.config.ts`, then the CMS host loads installed plugins at boot. Each server entrypoint runs in its own Bun `Worker`, and that worker hosts a **QuickJS-WASM sandbox** — no Node, no Bun, no host file system, no environment variables, no network unless explicitly granted. Plugins reach the CMS through SDK surfaces scoped to where they run: `api.plugin.*`, `api.cms.*`, `api.editor.*`, and `api.dashboard.*`.

---

## TL;DR

- **Package shape:** the CLI reads `ecobuilder-plugin.config.ts` and emits a zip containing `plugin.json` plus entrypoint bundles (`server/index.js`, `editor/index.js`, admin app bundles, `modules/index.js`, `frontend/*.js`, optional `pack/site.json`).
...
- **Permissions:** declared in `ecobuilder-plugin.config.ts` / `plugin.json`, approved by the site owner at install time, enforced by the SDK at runtime. Outbound network also requires a `networkAllowedHosts` allowlist.
- **CLI:** `bun ecobuilder-plugin init|lint|build|dev` covers scaffolding, manifest/source validation, bundle build, zip creation, and hot-sync to a running CMS.
...
| `ecobuilder-plugin` CLI                | `src/core/plugin-sdk/cli/`                |
...
`bun ecobuilder-plugin build` produces this runtime layout from `ecobuilder-plugin.config.ts`. Bundle formats are intentionally different per surface:
...
Authors normally write `ecobuilder-plugin.config.ts` with `definePlugin(...)`; the CLI emits the runtime `plugin.json`. The runtime manifest is still the install-time contract, and `parsePluginManifest` in `src/core/plugins/manifest.ts` is the source of truth.
...
`parsePluginManifest` validates all of these and produces a clear error message. `bun ecobuilder-plugin lint` runs the same checks before upload. Coherence checks also enforce permission-dependent shape:
...
- The manifest parser rejects an editor entrypoint or app-kind admin page that doesn't declare `editor.code` (`parsePluginManifest` coherence checks; `ecobuilder-plugin lint` reports the same error pre-upload).
...
1. **`ecobuilder-plugin build`** emits the correct bundle format for each surface and scans sandboxed bundles for the forbidden literals above.
2. **`ecobuilder-plugin lint`** runs the same scan plus manifest + permission/allowlist coherence checks. Run this before upload.
...
### Settings — declared in `ecobuilder-plugin.config.ts` / `plugin.json`
...
**Master-key rotation.** If `ECOBUILDER_SECRET_KEY` changes, stored plugin secrets can no longer be decrypted. The host does not crash plugin load — the worker simply sees those fields empty, and the server logs a `[plugin:<id>]` re-entry notice. The settings GET reports the affected field ids in `secretsNeedingReentry`, and the settings dialog shows a warning prompting the operator to re-enter them. Re-saving a value heals the row under the new key.
...
`bun ecobuilder-plugin <command>` runs the SDK CLI at `src/core/plugin-sdk/cli/` (`bun run ecobuilder-plugin <command>` works too inside this repo). The CLI's primary input is `ecobuilder-plugin.config.ts`.

```sh
bun ecobuilder-plugin init acme.hero    # scaffold a module plugin
bun ecobuilder-plugin init acme.seo --kind content-editor
bun ecobuilder-plugin lint              # validate config + manifest + sources + bundles
bun ecobuilder-plugin build             # produce dist/ + .plugin.zip
bun ecobuilder-plugin dev               # watch + sync into a running CMS
```
...
`ecobuilder-plugin dev` writes built files **directly** into the host's `uploads/plugins/<id>/<version>/`. Subsequent rebuilds are picked up on the next activation cycle.
...
```sh
ECOBUILDER_UPLOADS_DIR=../instatic/uploads bun ecobuilder-plugin dev
# or
bun ecobuilder-plugin dev --uploads ../instatic/uploads
```

First install still goes through the admin UI (`/admin/plugins` → Upload Plugin) so the owner approves permissions. Every `ecobuilder-plugin dev` rebuild after that flows in without another upload.
...
1. **Scaffold:**
   ```sh
   bun ecobuilder-plugin init my-plugin
   cd my-plugin
   ```
2. **Edit `ecobuilder-plugin.config.ts`.** Pick a namespaced ID (`vendor.product`), declare only the permissions you use, and add resources, settings, admin pages, modules, frontend assets, content access, or packs there.
...
5. **Lint:**
   ```sh
   bun ecobuilder-plugin lint
   ```
6. **Build:**
   ```sh
   bun ecobuilder-plugin build
   ```
7. **Install via admin UI** (`/admin/plugins` → Upload Plugin), approve permissions.
8. **Iterate** with `bun ecobuilder-plugin dev`.
...
| Calling a host capability without the matching permission                | Declare it in `ecobuilder-plugin.config.ts` / `plugin.json` permissions |
...
| Skipping `ecobuilder-plugin lint` before upload                                  | Always lint — the host scans anyway and refuses the upload   |
...
- Source-of-truth files:
  ...
  - `src/core/plugin-sdk/cli/` — `ecobuilder-plugin` CLI
