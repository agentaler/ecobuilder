...
- **Sessions** are token-cookie based. Cookie name: `SESSION_COOKIE_NAME` (`ecobuilder_admin_session`). Tokens are hashed before storage; the cookie carries the raw token.
...
- **First-run setup** (`POST /admin/api/cms/setup`) creates the site and its first owner, and 409s once one exists. In production it additionally requires a bootstrap token (`ECOBUILDER_SETUP_TOKEN`, or one generated per boot and logged) — a one-shot endpoint on a public hostname otherwise hands the install to whoever finds it first. See `server/auth/setupToken.ts`.
- **MFA (TOTP)** is per-user opt-in. TOTP seeds are encrypted at rest with `ECOBUILDER_SECRET_KEY`; recovery codes are one-way hashes. Sessions for MFA-enrolled users are `pending_mfa` until verified, then become `active`. Failed MFA codes go through `mfaRateLimit` AND increment the per-account lockout counter — the same counter the password step uses. A locked account is rejected at the MFA step before any code is checked.
...
createSession(user, deviceLabel, ip)
    ...
    │
    ▼
Set-Cookie: ecobuilder_admin_session=<rawToken>; HttpOnly; Secure; SameSite=Lax; Path=/admin
    │
    ▼
...
`POST /admin/api/cms/auth/logout` -> `revokeSessionByHash(db, hash)` -> `Set-Cookie: ecobuilder_admin_session=; Max-Age=0`.
