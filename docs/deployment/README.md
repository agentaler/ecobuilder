...
Ecobuilder is one Bun server packaged by the root `Dockerfile`. The server reads runtime configuration from `server/config.ts`: `PORT`, `DATABASE_URL`, `UPLOADS_DIR`, `STATIC_DIR`, `PUBLIC_ORIGIN`, and `TRUSTED_PROXY_CIDRS`. Reversible server secrets, including AI provider credentials, plugin secret settings, and MFA TOTP seeds, are encrypted with `ECOBUILDER_SECRET_KEY` when configured. Database migrations run automatically on boot in `server/index.ts`.
...
ECOBUILDER_SECRET_KEY  base64 32-byte key for encrypted server secrets
...
Generate `ECOBUILDER_SECRET_KEY` with `bun run scripts/generate-secret-key.ts` before adding Anthropic, OpenAI, or OpenRouter credentials or enabling TOTP MFA in production. Without it, the admin can load but saving reversible secrets fails because there is no stable encryption key.
...
```sh
ECOBUILDER_IMAGE=ghcr.io/agentaler/ecobuilder:0.0.13 docker compose -f compose.prod.yml -f compose.sqlite.yml up -d
```
...
The maintainer release target is `ghcr.io/agentaler/ecobuilder`, documented in [release-workflow.md](release-workflow.md).
