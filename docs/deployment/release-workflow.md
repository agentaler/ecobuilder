...
This maintainer guide covers publishing Ecobuilder Docker images.

End users do not need this page to deploy Ecobuilder. They follow [railway.md](railway.md), [render.md](render.md), [vps.md](vps.md), or [docker-image.md](docker-image.md). Maintainers use this page to keep `ghcr.io/agentaler/ecobuilder` release tags aligned with source tags and deployment templates.
...
```txt
ghcr.io/agentaler/ecobuilder:latest
ghcr.io/agentaler/ecobuilder:<semver>
ghcr.io/agentaler/ecobuilder:<major>.<minor>
```
...
The checked-in Render Blueprints use `ghcr.io/agentaler/ecobuilder:latest` for new one-click installs. `scripts/build-release-bundle.ts` rewrites the release-bundle copies to the semver image tag automatically.
...
```txt
agentaler/ecobuilder-render-sqlite
agentaler/ecobuilder-render-postgres
```
...
```txt
ghcr.io/agentaler/ecobuilder:0.0.1
ghcr.io/agentaler/ecobuilder:0.0
ghcr.io/agentaler/ecobuilder:latest
```
...
```txt
ecobuilder-0.0.1-release-bundle.tar.gz
```
...
GHCR (`ghcr.io/agentaler/ecobuilder`) is the only published registry. It is produced directly by the release workflow, is public, and has no aggressive anonymous pull-rate limits — use it in every Compose file, template, and deployment guide. There is no Docker Hub mirror; if one is ever wanted, add a `Mirror To Docker Hub` job plus `DOCKERHUB_USERNAME` / `DOCKERHUB_TOKEN` repository secrets.

## GHCR Visibility

After the first successful tagged release, open the package page for `ghcr.io/agentaler/ecobuilder` in GitHub Packages and set visibility to public.
