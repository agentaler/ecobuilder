...
This guide defines the Render Blueprint configuration for Ecobuilder.

Render can run Ecobuilder from the published Docker image, provide a public web service, attach a persistent disk, provision managed Postgres, and generate stable secret values at deploy time. The checked-in Blueprint templates live under `docs/deployment/render/` and are intended to be copied as the root `render.yaml` in dedicated Deploy to Render template repositories.
...
```txt
Image=ghcr.io/agentaler/ecobuilder:latest
PORT=10000
UPLOADS_DIR=/app/storage/uploads
STATIC_DIR=/app/dist
ECOBUILDER_SECRET_KEY=<generated by Render>
Health check path=/health
```

Render auto-injects `RENDER_EXTERNAL_URL` (the full public URL) into every web service at runtime. Ecobuilder uses it as the CSRF public origin, so no proxy or origin variable needs to be set for a one-click deploy.
...
|---|---|
| `agentaler/ecobuilder-render-sqlite` | `docs/deployment/render/sqlite/render.yaml` |
| `agentaler/ecobuilder-render-postgres` | `docs/deployment/render/postgres/render.yaml` |

Each template repo README can expose a button:

```md
[![Deploy to Render](https://render.com/images/deploy-to-render-button.svg)](https://render.com/deploy?repo=https://github.com/agentaler/ecobuilder-render-sqlite)
```
...
| Adding an AI provider credential or enabling TOTP MFA returns 500 | Confirm `ECOBUILDER_SECRET_KEY` exists and has not been rotated. |
