...
5. **Billing scope** (E10). SaaS implies paid plans; a Stripe connector is already attached to this workspace, so tickets assume Stripe. Plan structure/pricing is a business decision — E10-T01.
6. **`/_instatic/*` public namespaces** (E02). Renaming URL prefixes baked into published HTML requires dual-serving + republish. Tickets encode: rename env vars and CLI now; keep wire-level namespaces (`/_instatic/*`, `<instatic-hole>`, `@instatic/*` import specifiers) until a dedicated migration window — they are invisible to end users.
7. **Admin UI localization** is treated as out of scope (landing page only gets FR). Flag if the product itself must be bilingual.
