...
**Ecobuilder** is a commercial, closed-source CMS with a built-in visual editor and a first-class plugin system. One Bun server backed by either Postgres or SQLite (selected by `DATABASE_URL`). The output is intentionally plain, semantic HTML with hand-clean CSS — no framework runtimes injected into published pages.

The primary deployment target is the hosted SaaS at **app.ecobuilder.ai**. The product remains deployable as a self-contained single-server install, and existing self-hosted installations with real user data stay supported. A **multi-tenant conversion is planned** (see `tickets/`) — do not bake in new single-tenant assumptions, but do not speculatively build multi-tenant machinery outside those tickets either.

Note on naming: the E02 rename moved the machine-facing identifiers that could migrate safely to "Ecobuilder" names — `ECOBUILDER_*` env vars (legacy `INSTATIC_*` still read with a boot warning, `src/core/utils/renamedEnv.ts`), the `ecobuilder_admin_session` cookie (legacy name dual-read, `server/auth/tokens.ts`), `ecobuilder-*` localStorage keys (one-time boot sweep, `src/admin/lib/legacyStorageKeys.ts`), and the `ecobuilder-plugin` CLI (legacy config filename accepted with a warning). Identifiers baked into published HTML and built plugin bundles remain "Instatic" by deliberate deferral until a dual-serve + republish window: the `/_instatic/*` URL namespace (e.g. `/_instatic/mcp`), `<instatic-hole>` placeholders and `data-instatic-*` markup, and `@instatic/*` plugin specifiers. Docs must keep documenting the deferred identifiers exactly as they exist in code.
