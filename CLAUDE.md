...
Note on naming: the product surface says "Ecobuilder", but wire-level identifiers from the earlier "Instatic" name are intentionally retained until the E02 rename epic — `INSTATIC_*` env vars, the `/_instatic/*` URL namespace (e.g. `/_instatic/mcp`), `<instatic-hole>` placeholders, the `instatic_admin_session` cookie, `@instatic/*` plugin specifiers, `instatic-*` localStorage keys, and the `instatic-plugin` CLI. Docs must keep documenting these identifiers exactly as they exist in code.
