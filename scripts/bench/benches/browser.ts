...
const ADMIN_EMAIL_ENV = 'ECOBUILDER_BENCH_ADMIN_EMAIL'
const ADMIN_PASSWORD_ENV = 'ECOBUILDER_BENCH_ADMIN_PASSWORD'
  ...
  async run(ctx: BenchContext): Promise<BenchResult> {
    ...
    try {
      ...
      if (authOk && session) {
        ...
        const sessionCookie = cookies.find((c) => c.name === 'ecobuilder_admin_session')
